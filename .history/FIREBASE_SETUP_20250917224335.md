...
3. Optionnel : Activer **"Email link (passwordless sign-in)"** si souhaité
4. Cliquer sur **"Save"**
...
### Règles de développement (actuelles)

Les règles actuelles sont permissives pour le développement :

- Tous les utilisateurs connectés peuvent lire/écrire dans toutes les collections
- **⚠️ ATTENTION : Ne pas utiliser en production !**

### Règles de production

Pour la production, utiliser les règles dans `firestore.prod.rules` :

- Lecture pour tous les utilisateurs connectés
- Écriture réservée aux coaches
...
- `compositions` - Compositions d'équipes
- `availabilities` - Disponibilités des joueurs
- `burnRecords` - Historique des brûlages
...
4. **Ajouter des données de test** (joueurs, équipes, etc.)
5. **Tester les fonctionnalités** de l'application
...
# FFTT API Configuration (à remplir)
...
1. Vérifier que l'authentification Email/Password est activée
