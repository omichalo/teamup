...
async function testManualSync() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.log(`   Joueurs synchronisés: ${response.data.synced}/${response.data.total}`);
    ...
    if (response.status === 200) {
      ...
      if (response.data.logs.length > 0) {
        ...
        response.data.logs.forEach((log, index) => {
          console.log(`     ${index + 1}. ${log.type} - ${log.synced || 0} joueurs - ${log.timestamp?.toDate?.() || log.timestamp}`);
        });
      }
    } else {
