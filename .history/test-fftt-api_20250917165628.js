  ...
  try {
      ...
      if (response.ok) {
        ...
        if (matches.length > 0) {
          ...
          for (const match of matches.slice(0, 5)) {
            // Limiter à 5 matchs pour le test
            try {
              ...
              console.log(
                `   ✅ Match sauvegardé: ${match.equipe1} vs ${match.equipe2}`
              );
            } catch (error) {
        ...
        console.log(
          `❌ Erreur API matchs: ${response.status} ${response.statusText}`
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `❌ Erreur API joueur: ${response.status} ${response.statusText}`
        );
      }
    } catch (error) {
    ...
    for (const player of testPlayers) {
      try {
        ...
        console.log(
          `   ✅ Joueur créé: ${player.name} (${player.ranking} pts)`
        );
      } catch (error) {
        console.log(
          `   ❌ Erreur création joueur ${player.name}: ${error.message}`
        );
      }
    }
    ...
    for (const team of testTeams) {
      try {
        ...
        console.log(
          `   ❌ Erreur création équipe ${team.name}: ${error.message}`
        );
      }
    }
