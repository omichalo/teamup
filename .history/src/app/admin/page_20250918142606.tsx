...
export default function AdminPage() {
  ...
  React.useEffect(() => {
    const loadSyncStatus = async () => {
      if (!user) return; // Attendre que l'utilisateur soit connecté
      ...
      try {
        ...
        if (response.ok && result.success) {
          setSyncStatus({
            players: {
              lastSync: result.data.players.lastSync,
              count: result.data.players.count,
              status: "idle",
            },
            matches: {
              lastSync: result.data.matches.lastSync,
              count: result.data.matches.count,
              status: "idle",
            },
          });
        } else {
            ...
            <CardContent>
              ...
              <Box sx={{ pl: 2 }}>
                <Typography variant="body2" sx={{ mb: 1 }}>
                  • <strong>Joueurs :</strong> Synchronisation quotidienne à 6h
                  du matin
                </Typography>
                <Typography variant="body2" sx={{ mb: 1 }}>
