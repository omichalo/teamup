...
export default function DisponibilitesPage() {
  ...
  const [selectedPlayer, setSelectedPlayer] = useState<PlayerAvailability | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      const initialAvailabilities: PlayerAvailability[] = mockPlayers.map(player => ({
        playerId: player.id,
        player,
        availabilities: {},
        reasons: {},
  ...
  const handleAvailabilityChange = (playerId: string, journee: number, isAvailable: boolean) => {
    ...
    if (!isAvailable) {
      const player = playerAvailabilities.find(pa => pa.playerId === playerId);
      if (player) {
  ...
  const getAvailabilityStats = () => {
    ...
    journees.forEach(journee => {
      const available = playerAvailabilities.filter(pa => 
        pa.availabilities[journee] === true
      ).length;
    ...
    <Layout>
      <Box>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              {journees.map(journee => (
                <Chip
                  ...
                  color={stats[journee].available >= stats[journee].total * 0.8 ? 'success' : 'warning'}
                  variant="outlined"
                />
              ))}
            </Box>
                    ...
                    <TableRow key={pa.playerId}>
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          {pa.player.isForeign && (
                            <Chip label="Étranger" size="small" color="warning" />
                          )}
                        </Box>
                      </TableCell>
                      <TableCell>
                        Équipe {pa.player.teamNumber}
                      </TableCell>
                      <TableCell>
                        {pa.player.points}
                      </TableCell>
                          ...
                          <FormControlLabel
                            control={
                              <Switch
                                ...
                                color={getAvailabilityColor(pa.availabilities[journee])}
                              />
                            }
                            label=""
                          />
                          {pa.availabilities[journee] === false && pa.reasons[journee] && (
        ...
        <Dialog open={showReasonDialog} onClose={() => setShowReasonDialog(false)}>
          <DialogTitle>
            Raison d'indisponibilité - {selectedPlayer?.player.firstName} {selectedPlayer?.player.lastName}
          </DialogTitle>
          <DialogContent>
            <TextField
              autoFocus
