  ...
  return (
    <Box sx={{ p: 3 }}>
              ...
              <TableBody>
                {filteredPlayers.map((player) => (
                  <TableRow key={player.licence} hover>
                    ...
                    <TableCell>{getNationalityChip(player.natio)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      <Box sx={{ mt: 4, p: 2, backgroundColor: "grey.50", borderRadius: 1 }}>
        <Typography variant="body2" color="text.secondary">
          <strong>Note :</strong> Cette page affiche les données en temps réel
          depuis l'API FFTT. Les données sont mises à jour automatiquement lors
          de chaque actualisation.
        </Typography>
      </Box>
    </Box>
  );
