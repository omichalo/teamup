...
export default function TestFirestoreAuthPage() {
  ...
  const testFirestoreAccess = async () => {
    ...
    try {
      ...
      console.log("Test: Tentative d'accès à Firestore avec UID:", firebaseUser.uid);
      const userData = await getUser(firebaseUser.uid);
      
      if (userData) {
        setTestResult(`✅ Succès! Utilisateur trouvé: ${userData.email} (${userData.role})`);
      } else {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <Typography>Firebase User: {firebaseUser ? firebaseUser.email : "Non connecté"}</Typography>
        <Typography>User Data: {user ? `${user.email} (${user.role})` : "Non chargé"}</Typography>
      </Box>
