...
export default function CompositionsPage() {
  ...
  const loading = playersLoading || teamsLoading || journeesLoading;
  ...
  if (playersError || teamsError || journeesError) {
    return (
      <Layout>
        <Box sx={{ p: 3 }}>
          <Alert severity="error">
            Erreur lors du chargement des données: {playersError || teamsError || journeesError}
          </Alert>
        </Box>
      </Layout>
    );
    ...
    <Layout>
      <Box>
        ...
        <Typography variant="subtitle1" color="text.secondary" gutterBottom>
          {players.length} joueurs, {teams.length} équipes et {journees.length} journées chargés depuis FFTT
        </Typography>
          ...
          <Grid xs={12} md={4}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                disabled={journees.length === 0}
              >
                {journees.map((journee) => (
                  <MenuItem key={`${journee.phase}_${journee.number}`} value={journee.number}>
                    <Box>
                      <Typography variant="body1">{journee.name}</Typography>
          ...
          <Grid xs={12} md={4}>
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button
                ...
                disabled={!teamCompositions[selectedTeam]?.A || selectedJournee === null}
              >
                Valider
              </Button>
              <Button
