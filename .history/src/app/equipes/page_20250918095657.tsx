...
export default function EquipesPage() {
  ...
  const equipesMasculines = equipes.filter(equipe => 
    equipe.matches.length > 0 && !equipe.matches[0].isFemale
  );
  const equipesFeminines = equipes.filter(equipe => 
    equipe.matches.length > 0 && equipe.matches[0].isFemale
  );
      ...
      {equipes.length === 0 ? (
        ...
        <Box sx={{ mt: 3 }}>
          <Tabs value={tabValue} onChange={handleTabChange} sx={{ mb: 3 }}>
            <Tab 
              label={`Messieurs (${equipesMasculines.length})`} 
              icon={<SportsTennis />}
            />
            <Tab 
              label={`Dames (${equipesFeminines.length})`} 
              icon={<SportsTennis />}
            />
          </Tabs>

          {/* Affichage du libellé de l'épreuve */}
          {tabValue === 0 && equipesMasculines.length > 0 && (
            <Typography variant="h6" color="primary" sx={{ mb: 2 }}>
              {equipesMasculines[0].matches[0]?.division?.includes('Féminin') ? 
                'Championnat de France par Équipes Féminin' : 
                'Championnat de France par Équipes Masculin'
              }
            </Typography>
          )}
          {tabValue === 1 && equipesFeminines.length > 0 && (
          ...
          {tabValue === 0 && (
            <Box>
              {equipesMasculines.length === 0 ? (
                <Alert severity="info">Aucune équipe masculine trouvée.</Alert>
              ) : (
                equipesMasculines.map((equipeWithMatches) => (
          ...
          {tabValue === 1 && (
            <Box>
              {equipesFeminines.length === 0 ? (
                <Alert severity="info">Aucune équipe féminine trouvée.</Alert>
              ) : (
                equipesFeminines.map((equipeWithMatches) => (
            <Accordion key={equipeWithMatches.team.id} sx={{ mb: 2 }}>
              <AccordionSummary expandIcon={<ExpandMore />}>
                <Box
                  sx={{ display: "flex", alignItems: "center", width: "100%" }}
