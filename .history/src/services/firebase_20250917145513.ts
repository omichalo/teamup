...
export const addPlayer = async (player: Omit<Player, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  const now = new Date();
...
export const addTeam = async (team: Omit<Team, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  const now = new Date();
...
export const updateTeam = async (id: string, updates: Partial<Team>): Promise<void> => {
  const docRef = doc(teamsCollection, id);
  await updateDoc(docRef, {
    ...updates,
    updatedAt: Timestamp.fromDate(new Date()),
...
export const getMatchesByTeam = async (teamNumber: number): Promise<Match[]> => {
  const q = query(
    matchesCollection,
...
export const addMatch = async (match: Omit<Match, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  const now = new Date();
...
export const updateMatch = async (id: string, updates: Partial<Match>): Promise<void> => {
...
export const getCompositions = async (): Promise<Composition[]> => {
  const snapshot = await getDocs(query(compositionsCollection, orderBy('createdAt', 'desc')));
  return snapshot.docs.map(doc => ({
    id: doc.id,
    ...convertTimestamps(doc.data()),
  })) as Composition[];
...
export const getCompositionByMatch = async (matchId: string): Promise<Composition | null> => {
...
export const addComposition = async (composition: Omit<Composition, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  const now = new Date();
...
export const updateComposition = async (id: string, updates: Partial<Composition>): Promise<void> => {
...
export const getAvailabilitiesByJournee = async (journee: number): Promise<Availability[]> => {
...
export const addAvailability = async (availability: Omit<Availability, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  const now = new Date();
...
export const updateAvailability = async (id: string, updates: Partial<Availability>): Promise<void> => {
...
export const getBurnRecordsByPlayer = async (playerId: string): Promise<BurnRecord[]> => {
...
export const addBurnRecord = async (burnRecord: Omit<BurnRecord, 'id' | 'createdAt'>): Promise<string> => {
...
export const updateClubSettings = async (updates: Partial<ClubSettings>): Promise<void> => {
  const snapshot = await getDocs(clubSettingsCollection);
  if (!snapshot.empty) {
