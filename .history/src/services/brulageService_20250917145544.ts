import { Player, BurnRecord, CompositionValidation, ValidationError } from '@/types';
...
export class BrulageService {
  ...
  validateComposition(
    ...
    const burnErrors = this.validateBurnRules(players, teamNumber, journee, phase);
  ...
  private validateBurnRules(
    ...
    for (const player of players) {
      ...
      if (burnCount > 0) {
        const hasPlayedInLowerTeam = this.hasPlayedInLowerTeam(player.id, teamNumber, phase);
   ...
   */
  private validateDay2Quota(players: Player[], teamNumber: number): ValidationError[] {
    const errors: ValidationError[] = [];
   ...
   */
  private hasPlayedInLowerTeam(playerId: string, currentTeamNumber: number, phase: string): boolean {
    const playerRecords = this.burnRecords.filter(
      record => record.playerId === playerId && record.phase === phase
    );
    
    return playerRecords.some(record => record.teamNumber < currentTeamNumber);
  }
   ...
   */
  addBurnRecord(playerId: string, teamNumber: number, journee: number, phase: string, matchId: string): BurnRecord {
  ...
  resetBurnForPhase(phase: string): void {
    this.burnRecords = this.burnRecords.filter(record => record.phase !== phase);
  }
  ...
  getPlayerBurnStats(playerId: string): {
    ...
    const allMatches = this.burnRecords.filter(record => record.playerId === playerId);
