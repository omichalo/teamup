...
export class PlayerSyncService {
  ...
  private async enrichPlayersData(
    ...
    const launchNextRequest = (): Promise<FFTTJoueurDetails> | null => {
    ...
    while (activePromises.length > 0) {
      ...
      // Ajouter le résultat
      enrichedPlayers.push(completedPromise);
      
      // Retirer la promesse terminée du pool
      const completedIndex = activePromises.findIndex(p => p === completedPromise);
      if (completedIndex !== -1) {
        activePromises.splice(completedIndex, 1);
      }
      
      // Lancer une nouvelle requête si il reste des joueurs à traiter
      const nextPromise = launchNextRequest();
      if (nextPromise) {
