...
export const calculatePlayerBurnout = (
  ...
  return {
    playerLicence,
    playerName: sortedMatches[0].joueursSQY?.find(j => j.licence === playerLicence)?.nom || 'Inconnu',
    teamId,
    teamName,
    matchesPlayed,
    lastMatchDate,
...
export const calculateTeamBurnout = (
  ...
  playerLicences.forEach(licence => {
    const burnoutInfo = calculatePlayerBurnout(licence, teamId, teamName, matches, conditions);
    if (burnoutInfo) {
