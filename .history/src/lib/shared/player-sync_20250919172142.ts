...
export class PlayerSyncService {
  ...
  private async enrichPlayersData(
    ...
    console.log(
      `🔄 Enrichissement de ${joueurs.length} joueurs par batch de ${batchSize}...`
    );
    ...
    for (let i = 0; i < joueurs.length; i += batchSize) {
      ...
      console.log(
        `📦 Traitement du batch ${batchNumber}/${totalBatches} (${batch.length} joueurs)...`
      );
      ...
      try {
        ...
        console.log(
          `✅ Batch ${batchNumber}/${totalBatches} terminé (${batchResults.length} joueurs enrichis)`
        );
        ...
        const fallbackResults = batch.map((joueur: FFTTJoueur) =>
