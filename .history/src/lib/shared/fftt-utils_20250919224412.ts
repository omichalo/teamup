...
export const createBaseMatch = (
  ...
  if (detailsRencontre) {
    if (isHome) {
      joueursSQY = detailsRencontre.joueursA || [];
      joueursAdversaires = detailsRencontre.joueursB || [];
    } else {
      joueursSQY = detailsRencontre.joueursB || [];
      joueursAdversaires = detailsRencontre.joueursA || [];
    }
  }

  return {
    id: rencontre.lien, // Utiliser directement l'ID FFTT de la rencontre
    ...
    joueursSQY: joueursSQY.length > 0 ? joueursSQY : undefined,
    joueursAdversaires: joueursAdversaires.length > 0 ? joueursAdversaires : undefined,
  };
};
