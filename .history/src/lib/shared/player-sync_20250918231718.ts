...
export class PlayerSyncService {
  ...
  async syncPlayers(): Promise<PlayerSyncResult> {
    try {
      ...
      const enrichedPlayers = await this.enrichPlayersData(joueurs);
   ...
   */
  private async enrichPlayersData(joueurs: any[]): Promise<FFTTJoueurDetails[]> {
    ...
    for (let i = 0; i < joueurs.length; i += batchSize) {
      ...
      try {
        ...
        const batchPromises = batch.map(async (joueur: any) => {
          try {
        ...
        const fallbackResults = batch.map((joueur: any) => this.mergePlayerData(joueur, null));
   ...
   */
  private mergePlayerData(baseJoueur: any, details: any | null): FFTTJoueurDetails {
    ...
    if (details) {
      enrichedPlayer.classement = details.classement || details.classementGlobal;
      enrichedPlayer.categorie = details.categorie || details.cat;
      enrichedPlayer.nationalite = details.natio || details.nationalite;
      enrichedPlayer.dateNaissance = details.dateNaissance || details.dateNais;
      enrichedPlayer.lieuNaissance = details.lieuNaissance || details.lieuNais;
      enrichedPlayer.datePremiereLicence = details.datePremiereLicence || details.datePremiereLic;
      enrichedPlayer.clubPrecedent = details.clubPrecedent || details.clubPrec;
  ...
  async savePlayersToFirestore(
    ...
    try {
      console.log(
        `💾 Sauvegarde de ${players.length} joueurs dans Firestore...`
      );
