...
export class TeamMatchesSyncService {
  ...
  async syncMatchesForTeam(teamId: string): Promise<TeamMatchesSyncResult> {
    try {
      ...
      const equipe = equipes.find((eq: FFTTEquipe) => 
      ...
      console.log(`📊 ${rencontres.length} matchs trouvés pour ${equipe.libelle}`);

      // Transformer les matchs
      const processedMatches: MatchData[] = rencontres.map((rencontre: FFTTRencontre) =>
      ...
      console.error(`❌ Erreur lors de la synchronisation des matchs pour l'équipe ${teamId}:`, error);
      return {
        success: false,
        matchesCount: 0,
  ...
  async saveMatchesToTeamSubcollections(
    ...
    try {
      console.log(`💾 Sauvegarde de ${matches.length} matchs dans les sous-collections...`);

      // Grouper les matchs par équipe
      const matchesByTeam = new Map<string, MatchData[]>();
      
      matches.forEach((match) => {
        const teamId = `sqyping_team_${match.teamNumber}_${match.isFemale ? 'F' : 'M'}`;
      ...
      for (const [teamId, teamMatches] of matchesByTeam) {
        console.log(`💾 Sauvegarde de ${teamMatches.length} matchs pour ${teamId}...`);
        ...
        for (let i = 0; i < teamMatches.length; i += batchSize) {
          ...
          for (let j = i; j < batchEnd; j++) {
            ...
            const docRef = db.collection("teams").doc(teamId).collection("matches").doc(match.id);
