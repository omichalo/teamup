...
export class TeamMatchesSyncService {
  ...
  async saveMatchesToTeamSubcollections(
    ...
    try {
      ...
      // Créer un mapping des teamNumber vers les vrais IDs des équipes
      ...
      teamsSnapshot.docs.forEach((doc) => {
        ...
        teamMapping.set(key, doc.id);
      });

      matches.forEach((match) => {
        const key = `${match.teamNumber}_${match.isFemale ? "F" : "M"}`;
        const teamId = teamMapping.get(key);
        ...
        if (teamId) {
          ...
          console.warn(`⚠️ Équipe non trouvée pour ${key}`);
        }
      });
