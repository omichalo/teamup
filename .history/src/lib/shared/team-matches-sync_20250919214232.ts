...
export class TeamMatchesSyncService {
  ...
  async syncMatchesForTeam(teamId: string): Promise<TeamMatchesSyncResult> {
    try {
      ...
      const sqyPingMatches = rencontres.filter((rencontre: FFTTRencontre) =>
        rencontre.nomEquipeA.includes("SQY PING") || 
        rencontre.nomEquipeB.includes("SQY PING")
      );
      ...
      const processedMatches: MatchData[] = sqyPingMatches.map(
  ...
  private async fetchAllMatches(equipes: FFTTEquipe[]): Promise<MatchData[]> {
    const matchPromises = equipes.map(async (equipe) => {
      ...
      try {
        ...
        return sqyPingMatches.map((rencontre: FFTTRencontre) =>
          createBaseMatch(rencontre, equipe, this.clubCode)
        );
      } catch (error) {
  ...
  async saveMatchesToTeamSubcollections(
    ...
    try {
      ...
      // Créer un mapping des ffttId vers les vrais IDs des équipes
      const teamMapping = new Map<string, string>();
      
      // Récupérer toutes les équipes pour créer le mapping
      const teamsSnapshot = await db.collection("teams").get();
      teamsSnapshot.docs.forEach((doc) => {
        const teamData = doc.data();
        // Utiliser l'ID FFTT pour le mapping (plus fiable que teamNumber + isFemale)
        if (teamData.ffttId) {
          teamMapping.set(teamData.ffttId, doc.id);
        }
      });

      matches.forEach((match) => {
        // Extraire l'ID FFTT du match (il est dans l'ID du match)
        const matchIdParts = match.id.split('_');
        if (matchIdParts.length >= 2) {
          const ffttId = matchIdParts[1]; // Le deuxième élément est l'ID FFTT
          const teamId = teamMapping.get(ffttId);
          ...
          if (teamId) {
            ...
            console.warn(`⚠️ Équipe non trouvée pour FFTT ID ${ffttId}`);
          }
        } else {
          console.warn(`⚠️ Format d'ID de match invalide: ${match.id}`);
        }
      });
