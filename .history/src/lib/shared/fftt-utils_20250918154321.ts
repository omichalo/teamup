...
import { FFTTEquipe, FFTTJoueur, FFTTRencontre, FFTTDetailsRencontre } from "./fftt-types";
...
export const extractEquipeIds = (lien: string): { equipe1: string; equipe2: string } => {
...
export const enrichPlayerData = async (
  ...
  try {
    console.log(`🔍 Enrichissement des données joueurs pour le club ${clubCode}`);
    
    // Récupérer tous les joueurs du club
    const joueursClub = await ffttApi.getJoueursByClub(clubCode);
    console.log(`📋 ${joueursClub.length} joueurs trouvés pour le club ${clubCode}`);
    ...
    detailsRencontre.joueursA = detailsRencontre.joueursA.map((joueur) => {
      ...
      if (joueurClub) {
        console.log(`✅ Enrichissement: ${joueur.prenom} ${joueur.nom} -> licence: ${joueurClub.licence}, points: ${joueurClub.points}`);
        return {
          ...joueur,
    ...
    console.error("❌ Erreur lors de l'enrichissement des données joueurs:", error);
    return detailsRencontre;
  }
};
...
export const createBaseMatch = (
  ...
  return {
    ...
    score: rencontre.scoreEquipeA !== null && rencontre.scoreEquipeB !== null 
      ? `${rencontre.scoreEquipeA}-${rencontre.scoreEquipeB}` 
      : undefined,
    result: determineMatchResult(rencontre.scoreEquipeA, rencontre.scoreEquipeB, isHome),
