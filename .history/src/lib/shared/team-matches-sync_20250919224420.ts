    ...
    try {
      ...
      for (const rencontre of sqyPingMatches) {
        try {
          ...
          const detailsRencontre = await this.ffttApi.getDetailsRencontreByLien(rencontre.lien);
          
          const matchData = createBaseMatch(rencontre, equipe, this.clubCode, detailsRencontre);
          processedMatches.push(matchData);
        } catch (error) {
          console.warn(`⚠️ Impossible de récupérer les détails pour ${rencontre.lien}:`, error);
          // Créer le match sans les détails des joueurs
          const matchData = createBaseMatch(rencontre, equipe, this.clubCode);
          processedMatches.push(matchData);
        }
      }
