...
export default async function handler(
  ...
  try {
    console.log("🔄 Test de synchronisation des équipes (avec ffttId)...");
    ...
    const syncResult = await teamSyncService.syncTeams();
    ...
    const saveResult = await teamSyncService.saveTeamsToFirestore(
    ...
    res.status(200).json({
      success: true,
      message: "Synchronisation des équipes réussie (avec ffttId)",
      data: saveResult,
    });
  } catch (error) {
