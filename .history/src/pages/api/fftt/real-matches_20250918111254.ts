    ...
    for (const equipe of equipesFiltered) {
      try {
        ...
        for (let index = 0; index < rencontres.length; index++) {
          ...
          // Vérifier si le match a des scores (même si scoreEquipeA/scoreEquipeB sont null, 
          // ils peuvent être des nombres valides)
          const hasScore = (rencontre.scoreEquipeA !== null && rencontre.scoreEquipeB !== null) ||
                          (typeof rencontre.scoreEquipeA === 'number' && typeof rencontre.scoreEquipeB === 'number');
