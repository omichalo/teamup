              ...
              try {
                ...
                if (clubnum1Match && clubnum2Match) {
                  try {
                    ...
                    console.log(
                      `🔍 ANALYSE API - ${equipe.libelle} (${
                        equipe.division.includes("Dames")
                          ? "FÉMININ"
                          : "MASCULIN"
                      })`
                    );
                    console.log(
                      `📋 Paramètres API: lien=${rencontre.lien}, clubA=${clubEquipeA}, clubB=${clubEquipeB}`
                    );
                    ...
                    if (detailsRencontre?.joueursA) {
                      ...
                      Object.keys(detailsRencontre.joueursA).forEach(
                        (playerKey) => {
                          const player = detailsRencontre.joueursA[playerKey];
                          console.log(
                            `   - ${player.nom} ${player.prenom}: licence="${player.licence}", points=${player.points}, sexe="${player.sexe}"`
                          );
                        }
                      );
                    }
                    ...
                    if (detailsRencontre?.joueursB) {
                      ...
                      Object.keys(detailsRencontre.joueursB).forEach(
                        (playerKey) => {
