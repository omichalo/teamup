          ...
          batch.map(async ({ equipe, rencontre, index }) => {
            try {
              ...
              try {
                ...
                if (clubnum1Match && clubnum2Match) {
                  try {
                    console.log(`Tentative 1 - Appel API avec clubs: ${clubEquipeA} vs ${clubEquipeB}`);
                    detailsRencontre = await ffttApi.getDetailsRencontreByLien(
                      rencontre.lien,
                      clubEquipeA,
                      clubEquipeB
                    );
                    console.log(`✅ Détails récupérés avec succès (clubs) pour ${equipe.libelle}`);
                  } catch (error) {
                    lastError = error;
                    console.log(`❌ Erreur avec les clubs pour ${equipe.libelle}:`, error.message);
                  }
                }
                ...
                if (!detailsRencontre && equipeIds) {
                  try {
                    console.log(`Tentative 2 - Appel API avec IDs équipe: ${equipeIds.equipe1} vs ${equipeIds.equipe2}`);
                    detailsRencontre = await ffttApi.getDetailsRencontreByLien(
                      rencontre.lien,
                      equipeIds.equipe1,
                      equipeIds.equipe2
                    );
                    console.log(`✅ Détails récupérés avec succès (IDs équipe) pour ${equipe.libelle}`);
                  } catch (error) {
                    lastError = error;
                    console.log(`❌ Erreur avec les IDs équipe pour ${equipe.libelle}:`, error.message);
                  }
                }
                ...
                // Debug spécifique pour l'équipe 1 féminine
                if (equipe.libelle.includes("SQY PING 1") && equipe.division.includes("Dames")) {
                  console.log(`🔍 DEBUG ÉQUIPE 1 FÉMININE:`, {
                    lien: rencontre.lien,
                    clubnum1Match: clubnum1Match?.[1],
                    clubnum2Match: clubnum2Match?.[1],
                    equip1Match: equip1Match?.[1],
                    equip2Match: equip2Match?.[1],
                    clubEquipeA,
                    clubEquipeB,
                    equipeIds
                  });
