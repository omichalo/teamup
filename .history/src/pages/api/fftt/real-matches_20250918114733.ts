...
export default async function handler(
  ...
  try {
    ...
    console.log(`🚀 Parallélisation de ${equipesFiltered.length} appels API...`);
    ...
    // 🚀 ÉTAPE 3: Paralléliser la récupération des détails pour les matchs terminés
    if (matchesToProcess.length > 0) {
      console.log(`🚀 Parallélisation de ${matchesToProcess.length} appels de détails...`);
      
      const detailsResults = await Promise.all(
        matchesToProcess.map(async ({ equipe, rencontre, index }) => {
          try {
            ...
            try {
              ...
              console.log(`Tentative d'appel API avec clubs: ${clubEquipeA} vs ${clubEquipeB}`);
              ...
              console.log(`✅ Détails récupérés avec succès pour ${equipe.libelle}`);
            ...
            console.log(`Erreur lors de la création des détails du match terminé:`, error);
            return { index, detailsRencontre: null };
          }
        })
      );
      
      console.log(`✅ Tous les appels de détails terminés !`);
    ...
    console.log(`🔥🔥🔥 MATCHS AVEC SCORES DÉTECTÉS: ${matchesWithScores} 🔥🔥🔥`);
...
function determineMatchResult(rencontre: { scoreEquipeA: number | null; scoreEquipeB: number | null }, isHome: boolean): string {
...
function createBaseMatch(equipe: unknown, rencontre: unknown, teamNumber: number, phase: string): Match | null {
  // Utiliser les vraies données des équipes de l'API FFTT
  const equip1 = (rencontre as any).nomEquipeA || "";
  const equip2 = (rencontre as any).nomEquipeB || "";
