    ...
    for (const equipe of equipes) {
      try {
        ...
        for (const rencontre of rencontres) {
          ...
          // Déterminer la journée - extraire depuis le lien ou utiliser une valeur par défaut
          let journee = 1;
          const journeeMatch = lien.match(/journee=(\d+)/);
          if (journeeMatch) {
            journee = parseInt(journeeMatch[1]);
          } else if (rencontre.journee) {
            journee = rencontre.journee;
          }
