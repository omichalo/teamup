...
export default async function handler(
  ...
  try {
    ...
    // Récupérer les détails du club pour obtenir les épreuves
    const clubDetails = await ffttApi.getClubDetails(clubCode as string);
    
    // Pour l'instant, retourner les détails du club
    const matches = [{
      ffttId: `club_${clubCode}`,
      teamNumber: 1,
      opponent: clubDetails.nom || "Club inconnu",
      opponentClub: clubDetails.nom || "Club inconnu",
      date: new Date(),
      location: clubDetails.ville || "Lieu inconnu",
      isHome: true,
      isExempt: false,
      isForfeit: false,
      phase: "aller",
