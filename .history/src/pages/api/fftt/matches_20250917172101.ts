...
export default async function handler(
  ...
  try {
    ...
    // Pour l'instant, créer des données de test basées sur le club
    const matches = [{
      ffttId: `club_${clubCode}`,
      teamNumber: 1,
      opponent: clubDetails.nom || "Club SQY Ping",
      opponentClub: clubDetails.nom || "Club SQY Ping",
      date: new Date(),
      location: clubDetails.ville || "SQY Ping",
      isHome: true,
      isExempt: false,
      isForfeit: false,
      phase: "aller",
