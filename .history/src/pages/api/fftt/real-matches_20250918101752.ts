    ...
    for (const equipe of equipesFiltered) {
      try {
        ...
        for (let index = 0; index < rencontres.length; index++) {
          ...
          if (index === 0) { // Seulement pour le premier match pour éviter le spam
          ...
          const match: Match = {
            ...
            score: rencontre.scoreEquipeA && rencontre.scoreEquipeB 
              ? `${rencontre.scoreEquipeA}-${rencontre.scoreEquipeB}` 
              : undefined,
            result: determineMatchResult(rencontre, isHome),
            composition: rencontre.composition || undefined,
          };
...
function determineMatchResult(rencontre: any, isHome: boolean): string {
  ...
  if (rencontre.scoreEquipeA === "EXEMPT" || rencontre.scoreEquipeB === "EXEMPT") {
    return "EXEMPT";
  }
