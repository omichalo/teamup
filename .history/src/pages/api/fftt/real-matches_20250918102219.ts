      ...
      try {
        ...
        for (let index = 0; index < rencontres.length; index++) {
          ...
          try {
            ...
            if (rencontreId) {
              console.log(
                `Tentative de récupération des résultats individuels pour rencontre ${rencontreId}`
              );
              // Essayer différentes méthodes pour récupérer les résultats individuels
              resultatsIndividuels = await ffttApi.getResultatsIndividuels(
                rencontreId
              );
              console.log(
                `Résultats individuels récupérés:`,
                resultatsIndividuels
              );
            }
          } catch (error) {
            console.log(
              `Erreur lors de la récupération des résultats individuels:`,
              error
            );
          }
