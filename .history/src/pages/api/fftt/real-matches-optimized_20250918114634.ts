  ...
  try {
    ...
    if (matchesToProcess.length > 0) {
      ...
      const detailsResults = await Promise.all(
        matchesToProcess.map(async ({ equipe, rencontre, index, teamNumber, phase }) => {
          try {
...
function createBaseMatch(equipe: any, rencontre: any, teamNumber: number, phase: string): Match {
