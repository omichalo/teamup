              ...
              try {
                ...
                if (clubnum1Match && clubnum2Match) {
                  try {
                    ...
                    console.log(`🔍 ANALYSE API - ${equipe.libelle} (${equipe.division.includes('Dames') ? 'FÉMININ' : 'MASCULIN'})`);
                    console.log(`📋 Paramètres API: lien=${rencontre.lien}, clubA=${clubEquipeA}, clubB=${clubEquipeB}`);
                    console.log(`📊 Structure des données retournées:`);
                    console.log(`   - joueursA:`, detailsRencontre?.joueursA ? Object.keys(detailsRencontre.joueursA).length + ' joueurs' : 'AUCUN');
                    console.log(`   - joueursB:`, detailsRencontre?.joueursB ? Object.keys(detailsRencontre.joueursB).length + ' joueurs' : 'AUCUN');
                    
                    // Analyser les licences et points pour chaque joueur
                    if (detailsRencontre?.joueursA) {
                      console.log(`🔍 Joueurs équipe A (${equipe.libelle}):`);
                      Object.keys(detailsRencontre.joueursA).forEach(playerKey => {
                        const player = detailsRencontre.joueursA[playerKey];
                        console.log(`   - ${player.nom} ${player.prenom}: licence="${player.licence}", points=${player.points}, sexe="${player.sexe}"`);
                      });
                    }
                    
                    if (detailsRencontre?.joueursB) {
                      console.log(`🔍 Joueurs équipe B (${equipe.libelle}):`);
