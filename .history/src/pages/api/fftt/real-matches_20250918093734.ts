    ...
    for (const equipe of equipes) {
      try {
        ...
        for (const rencontre of rencontres) {
          ...
          // Parser la date - améliorer la logique
          let matchDate: Date;
          if (rencontre.dateprevi && rencontre.dateprevi !== "0000-00-00") {
            matchDate = new Date(rencontre.dateprevi);
          } else if (rencontre.daterencontre && rencontre.daterencontre !== "0000-00-00") {
            matchDate = new Date(rencontre.daterencontre);
          } else {
            // Calculer une date réaliste basée sur la saison et la journée
            ...
            // Ajouter un jour aléatoire de la semaine (vendredi, samedi, dimanche)
            const dayOfWeek = Math.floor(Math.random() * 3) + 5; // 5=vendredi, 6=samedi, 7=dimanche
            const currentDay = matchDate.getDay();
            const daysToAdd = dayOfWeek - currentDay;
            matchDate.setDate(matchDate.getDate() + daysToAdd);
            // Ajouter une heure réaliste (14h, 15h, 16h, 20h)
            const hours = [14, 15, 16, 20][Math.floor(Math.random() * 4)];
