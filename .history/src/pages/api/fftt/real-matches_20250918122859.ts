          ...
          batch.map(async ({ equipe, rencontre, index }) => {
            try {
              ...
              try {
                ...
                if (detailsRencontre && detailsRencontre.joueursA && detailsRencontre.joueursB) {
                  detailsRencontre = await enrichPlayerData(detailsRencontre, clubCode as string);
                }
      ...
      if (detailsRencontre.joueursA) {
        Object.keys(detailsRencontre.joueursA).forEach(playerKey => {
          ...
          if (clubPlayer && (!player.licence || !player.points)) {
            console.log(`✅ Enrichissement: ${player.nom} ${player.prenom} -> licence: ${clubPlayer.licence}, points: ${clubPlayer.points}`);
