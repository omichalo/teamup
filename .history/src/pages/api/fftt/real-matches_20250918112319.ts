      ...
      try {
        ...
        for (let index = 0; index < rencontres.length; index++) {
          ...
          if (hasScore) {
            ...
            try {
              ...
              if (equipeIds) {
                ...
                try {
                  // Extraire le numéro de club depuis le lien (clubnum_1 ou clubnum_2)
                  const clubnum1Match = rencontre.lien.match(/clubnum_1=([^&]+)/);
                  const clubnum2Match = rencontre.lien.match(/clubnum_2=([^&]+)/);
                  
                  // Utiliser le club de SQY PING (08781477)
                  let clubNumber = "08781477"; // Club SQY PING par défaut
                  if (clubnum1Match && clubnum1Match[1] === "08781477") {
                    clubNumber = clubnum1Match[1];
                  } else if (clubnum2Match && clubnum2Match[1] === "08781477") {
                    clubNumber = clubnum2Match[1];
                  }
                  
                  console.log(`Tentative d'appel API avec club: ${clubNumber}`);
                  ...
                  detailsRencontre = await ffttApi.getDetailsRencontreByLien(
                    ...
                    equipeIds.equipe1,
                    equipeIds.equipe2,
                    clubNumber // Ajouter le paramètre club manquant
                  );
                  
                  console.log(`✅ Détails récupérés avec succès pour ${equipe.libelle}`);
                } catch (apiError) {
                  console.log(`❌ Erreur API, création d'un objet basique:`, apiError);
                ...
                console.log(
                  `Détails créés pour match terminé:`,
                  detailsRencontre
                );
              }
            } catch (error) {
          ...
          if (
            ...
            (rencontre as any).scoreA ||
            (rencontre as any).scoreB ||
            (rencontre as any).res_1 ||
            (rencontre as any).res_2
          ) {
