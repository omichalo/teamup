...
export default async function handler(
  ...
  try {
    console.log("🔄 Déclenchement de la synchronisation des matchs via Cloud Function");

    // Appeler la Cloud Function pour synchroniser les matchs
    const functionUrl = `https://us-central1-${process.env.FIREBASE_PROJECT_ID || "sqyping-teamup"}.cloudfunctions.net/triggerMatchSync`;
