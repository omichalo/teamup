...
export default async function handler(
  ...
  try {
    console.log("🔄 Synchronisation manuelle des matchs déclenchée");
    ...
    console.log(`📋 Synchronisation des matchs pour le club ${clubCode}`);

    // Récupérer les équipes du club
    const equipes = await ffttApi.getEquipesByClub(clubCode);
    ...
    const equipesFiltered = equipes.filter(
      (equipe: any) => equipe.idEpreuve === 15954 || equipe.idEpreuve === 15955
    ) as FFTTEquipe[];
    ...
    if (matchesToProcess.length > 0) {
      console.log(`🔥 ${matchesToProcess.length} matchs avec scores à traiter`);
...
function extractEquipeIds(lien: string): { equipe1: string; equipe2: string } | undefined {
...
async function enrichPlayerData(detailsRencontre: any, clubCode: string): Promise<any> {
  try {
    const ffttApi = new FFTTAPI(
      process.env.ID_FFTT || "SW251",
      process.env.PWD_FFTT || "XpZ31v56Jr"
    ...
    console.log(`❌ Erreur lors de l'enrichissement des données joueurs:`, error);
    return detailsRencontre;
  }
}
...
function createBaseMatch(
  ...
  const sqyPingInMatch = equip1.includes("SQY PING") || equip2.includes("SQY PING");
  if (!sqyPingInMatch) {
    return null;
  }
