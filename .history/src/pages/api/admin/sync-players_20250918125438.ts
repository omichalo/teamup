...
export default async function handler(
  ...
  try {
    console.log("🔄 Déclenchement de la synchronisation des joueurs via Cloud Function");

    // Appeler la Cloud Function pour synchroniser les joueurs
    const functionUrl = `https://us-central1-${process.env.FIREBASE_PROJECT_ID || "sqyping-teamup"}.cloudfunctions.net/triggerPlayerSync`;
