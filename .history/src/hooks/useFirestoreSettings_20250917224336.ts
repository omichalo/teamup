  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        console.log(
          "🔄 useFirestoreSettings: Exécution de la requête Firestore..."
        );
        const querySnapshot = await getDocs(q);
        console.log(
          "✅ useFirestoreSettings: Requête terminée,",
          querySnapshot.size,
          "documents trouvés"
        );
        ...
        if (querySnapshot.empty) {
          console.log(
            "⚠️ useFirestoreSettings: Aucun paramètre trouvé, création des paramètres par défaut"
          );
