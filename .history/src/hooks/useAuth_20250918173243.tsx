      ...
      if (firebaseUser && firebaseUser.uid) {
        try {
          ...
          if (error.code === 'permission-denied' || error.message?.includes('Missing or insufficient permissions')) {
            console.log("Hook useAuth: Erreur de permissions détectée, tentative de création d'utilisateur...");
            try {
              const newUser: Omit<User, "id" | "createdAt" | "updatedAt"> = {
                email: firebaseUser.email!,
                displayName: firebaseUser.displayName || "",
              ...
              console.log("Hook useAuth: Utilisateur créé après erreur de permissions:", userData);
              setUser(userData);
              return;
            } catch (createError) {
              console.error("Hook useAuth: Erreur lors de la création après permissions:", createError);
            }
          }
    ...
    try {
      ...
      if (firebaseUser) {
        try {
          ...
          // Si c'est une erreur de permissions, essayer de créer l'utilisateur quand même
          if (error.code === 'permission-denied' || error.message?.includes('Missing or insufficient permissions')) {
            console.log("Hook useAuth: Erreur de permissions détectée dans signIn, tentative de création d'utilisateur...");
            try {
              const newUser: Omit<User, "id" | "createdAt" | "updatedAt"> = {
                email: firebaseUser.email!,
                displayName: firebaseUser.displayName || "",
              ...
              await addUser(newUser, firebaseUser.uid);
              const userData = await getUser(firebaseUser.uid);
              console.log("Hook useAuth: Utilisateur créé après erreur de permissions dans signIn:", userData);
              setUser(userData);
              setFirebaseUser(firebaseUser);
              return;
            } catch (createError) {
              console.error("Hook useAuth: Erreur lors de la création après permissions dans signIn:", createError);
            }
          }
