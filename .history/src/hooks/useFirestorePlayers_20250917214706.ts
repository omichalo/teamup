  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      try {
        ...
        console.log("🔄 useFirestorePlayers: Création de la référence collection...");
        const playersRef = collection(db, "players");
        const q = query(playersRef, limit(maxPlayers));

        console.log("🔄 useFirestorePlayers: Exécution de la requête Firestore...");
        const querySnapshot = await getDocs(q);
        console.log("✅ useFirestorePlayers: Requête terminée,", querySnapshot.size, "documents trouvés");
        ...
        console.log("✅ useFirestorePlayers: Données traitées,", players.length, "joueurs");
        setData({
          players,
