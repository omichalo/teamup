  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        ...
        console.log("🔄 useFirestoreTeams: Création de la référence collection...");
        const teamsRef = collection(db, "teams");
        const q = query(teamsRef, orderBy("number", "asc"), limit(maxTeams));

        console.log("🔄 useFirestoreTeams: Exécution de la requête Firestore...");
        const querySnapshot = await getDocs(q);
        console.log("✅ useFirestoreTeams: Requête terminée,", querySnapshot.size, "documents trouvés");
        ...
        console.log("✅ useFirestoreTeams: Données traitées,", teams.length, "équipes");
        setData({
          teams,
          loading: false,
          error: null,
