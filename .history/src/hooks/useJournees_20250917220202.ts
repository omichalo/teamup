  ...
  useEffect(() => {
    const fetchJournees = async () => {
      try {
        ...
        const response = await fetch(`/api/fftt/real-matches?clubCode=${clubCode}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        
        const matches: Match[] = await response.json();
        console.log(`📊 ${matches.length} matches récupérés pour calculer les journées`);
...
function calculateJournees(matches: Match[]): Journee[] {
  ...
  const journeesMap = new Map<string, {
    number: number;
    phase: string;
    dates: Set<string>;
    matches: Match[];
  ...
  matches.forEach((match) => {
    ...
    const dateKey = match.date.toISOString().split('T')[0]; // YYYY-MM-DD
  ...
  const journees: Journee[] = Array.from(journeesMap.values())
    .map((journee) => ({
      ...
      phase: journee.phase,
      matches: journee.matches.sort((a, b) => a.date.getTime() - b.date.getTime()),
    }))
    .sort((a, b) => {
