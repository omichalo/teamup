...
import { TeamMatchesSyncService } from "../shared/team-matches-sync";
...
export const syncTeamMatchesDaily = functions.pubsub
  ...
  .timeZone("Europe/Paris")
  .onRun(async () => {
    console.log("🏆 Synchronisation quotidienne des matchs par équipe SQY Ping démarrée");
 ...
 * Déclenchée via HTTP POST
 */
export const syncTeamMatchesManual = functions.https.onRequest(async (req, res) => {
  // Configurer CORS
  res.set("Access-Control-Allow-Origin", "*");
  res.set("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
  res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
  ...
  try {
    console.log("🏆 Synchronisation manuelle des matchs par équipe SQY Ping démarrée");
    const result = await syncTeamMatchesFunction();
    res.status(200).json(result);
  } catch (error) {
    console.error("❌ Erreur synchronisation manuelle des matchs par équipe:", error);
    res.status(500).json({
      error: "Erreur lors de la synchronisation",
      details: error instanceof Error ? error.message : "Unknown error",
    });
...
async function syncTeamMatchesFunction() {
  try {
    ...
    console.error("❌ Erreur lors de la synchronisation des matchs par équipe:", error);
    return {
      success: false,
      matchesCount: 0,
      message: "Erreur lors de la synchronisation",
