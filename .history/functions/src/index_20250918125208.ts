...
// Fonction pour synchroniser les joueurs
export const syncPlayers = functions.pubsub
  .schedule("0 1 * * *") // Tous les jours à 1h du matin
  .timeZone("Europe/Paris")
  .onRun(async (context) => {
    console.log("🚀 Début de la synchronisation des joueurs FFTT");
...
// Fonction pour déclencher manuellement la synchronisation des joueurs
export const triggerPlayerSync = functions.https.onRequest(async (req, res) => {
  if (req.method !== "POST") {
  ...
  try {
    ...
    await syncPlayers.run({} as any, {} as any);
    ...
    console.error("❌ Erreur lors de la synchronisation manuelle des joueurs:", error);
    res.status(500).json({
