...
// Interface pour les joueurs FFTT
...
export const syncPlayersWeekly = functions.pubsub
  ...
  .timeZone("Europe/Paris")
  .onRun(async (context) => {
    console.log("🔄 Synchronisation hebdomadaire des joueurs SQY Ping démarrée");
    return await syncPlayers();
  });
...
async function syncPlayers() {
  ...
  try {
    ...
    console.log(`📦 Traitement en ${batches.length} batches de ${batchSize} joueurs`);

    for (const batch of batches) {
      const batchPromises = batch.map(async (ffttPlayer: FFTTPlayer) => {
        try {
 ...
 */
export const testFFTTConnection = functions.https.onRequest(async (req, res) => {
  try {
