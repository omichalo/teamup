...
export const cleanupDuplicatePlayers = functions.https.onRequest(async (req, res) => {
  // Configurer CORS
  ...
  try {
    ...
    console.log(`📊 ${playersSnapshot.size} documents trouvés dans la collection players`);
    ...
    playersSnapshot.forEach(doc => {
      ...
      if (doc.id === licence) {
        if (playersByLicence.has(licence)) {
          console.log(`⚠️ Doublon trouvé pour licence ${licence}, document ${doc.id} à supprimer`);
          duplicatesToDelete.push(doc.id);
        } else {
