...
export class MatchSyncService {
  ...
  async syncMatches(): Promise<MatchSyncResult> {
    try {
      ...
      console.log(
        `📋 ${equipes.length} équipes trouvées pour le club ${this.clubCode}`
      );
      ...
      console.log(
        `🚀 Parallélisation de ${filteredEquipes.length} appels API...`
      );
      ...
      console.log(
        `🔥🔥🔥 MATCHS AVEC SCORES DÉTECTÉS: ${matchesWithScores.length} 🔥🔥🔥`
      );
      ...
      console.log(
        `✅ Synchronisation terminée: ${allMatches.length} matchs récupérés`
      );
      return {
        success: true,
  ...
  private async processMatchDetails(matchesWithScores: any[]): Promise<void> {
    ...
    console.log(
      `🚀 Traitement par batch de ${matchesWithScores.length} appels de détails...`
    );
    ...
    for (let i = 0; i < matchesWithScores.length; i += BATCH_SIZE) {
      ...
      console.log(
        `📦 Traitement du batch ${batchNumber}/${totalBatches} (${batch.length} appels)`
      );

      const batchPromises = batch.map(async (match) => {
        try {
          console.log(
            `Récupération des détails pour match terminé: ${match.rencontreId}`
          );
          ...
          console.log(
            `✅ Détails récupérés avec succès pour ${match.rencontreId}`
          );

          return match;
        } catch (error) {
          console.error(
            `❌ Erreur lors de la récupération des détails pour ${match.rencontreId}:`,
            error
          );
          return match;
        }
  ...
  async saveMatchesToFirestore(
    ...
    try {
      console.log(
        `💾 Sauvegarde de ${matches.length} matchs dans Firestore...`
      );
