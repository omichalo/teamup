...
export default function DefaultCompositionsPage() {
  ...
  const { players, loadingPlayers, loadPlayers, playerPool } =
  ...
  // discordMembers est maintenant chargé automatiquement par les composants via le store
  ...
  useEffect(() => {
    if (!selectedPhase) {
      ...
      console.log(
        "[DefaultCompositions] Phase non sélectionnée, réinitialisation des compositions"
      );
      return;
    }
    ...
    const loadDefaults = async () => {
      try {
        ...
        console.log(
          "[DefaultCompositions] Échec du chargement des compositions, réinitialisation",
          {
            phase: selectedPhase,
          }
        );
  ...
  const {
    currentTab: defaultCompositionTab,
    currentTeams,
    championshipType: championshipTypeForAssignments,
    handleTabChange,
  } = useChampionshipTab({
    isParis,
    equipesByType,
  ...
  const assignPlayerToTeam = useCallback(
    async (teamId: string, playerId: string): Promise<boolean> => {
      ...
      if (currentTeamPlayers.includes(playerId)) {
        ...
        if (removedFromOtherTeams) {
          ...
          try {
            ...
            console.log("[DefaultCompositions] Persisting removal failed", {
              teamId,
              playerId,
              championshipType,
              phase: selectedPhase,
      ...
      try {
        console.log(
          "[DefaultCompositions] Sauvegarde d'une composition par défaut",
          {
            teamId,
            playerId,
            championshipType,
  ...
  const handleRemoveDefaultPlayer = useCallback(
    async (teamId: string, playerId: string) => {
      ...
      setDefaultCompositions((prev) => {
        ...
        console.log(
          "[DefaultCompositions] Suppression d'un joueur de la composition",
          {
            teamId,
            playerId,
            championshipType,
      ...
      try {
        console.log(
          "[DefaultCompositions] Sauvegarde après suppression d'un joueur",
          {
            teamId,
            playerId,
            championshipType,
        ...
        console.log(
          "[DefaultCompositions] Sauvegarde réussie après suppression",
          {
            teamId,
            playerId,
            championshipType,
        ...
        console.log(
          "[DefaultCompositions] Échec de la sauvegarde après suppression",
          {
            teamId,
            playerId,
            championshipType,
  ...
  const availablePlayersSubtitle = useMemo(() => {
    const base = defaultCompositionTab === "masculin" ? "Masculin" : "Féminin";
    ...
    >
      <Box sx={{ p: 5 }}>
        ...
        <CompositionSelectors
          ...
          onPhaseChange={(phase) =>
            setSelectedPhase(phase ? (phase as "aller" | "retour") : null)
          }
          onJourneeChange={() => {}}
          isParis={isParis}
