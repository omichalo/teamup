...
const CSRF_TTL = 24 * 60 * 60 * 1000; // 24 heures

/**
 * Génère un token CSRF signé basé sur l'UID de l'utilisateur et un secret.
 * Utilise HMAC-SHA256 pour garantir l'intégrité et prévenir la fuite du secret.
 */
...
/**
 * Valide un token CSRF en vérifiant sa signature HMAC, son expiration et sa liaison à l'utilisateur.
...
export async function validateCSRFToken(
  ...
  try {
    ...
    // Le token doit correspondre au cookie (Double Submit Cookie)
    ...
    if (!tokenUid || !timestampStr || !signature) return false;

    // 1. Liaison à l'utilisateur
    if (uid && tokenUid !== uid) return false;

    // 2. Expiration (TTL)
    const timestamp = parseInt(timestampStr, 10);
    if (isNaN(timestamp) || Date.now() - timestamp > CSRF_TTL) return false;

    // 3. Signature cryptographique
    const data = `${tokenUid}:${timestampStr}`;
