 ...
 * 🛡️ Sentinel: Utilise HMAC-SHA256 pour éviter la fuite du secret dans le token.
 */
export async function generateCSRFToken(uid: string): Promise<string> {
  ...
  // Le token contient l'UID, le timestamp et le HMAC des deux
 ...
 * 🛡️ Sentinel: Utilise timingSafeEqual pour prévenir les attaques par canal auxiliaire temporel.
 */
export async function validateCSRFToken(
  ...
  try {
    ...
    try {
      ...
      const [tokenUid, timestamp, providedHmac] = decoded.split(":");
      
      if (!tokenUid || !timestamp || !providedHmac) {
        return false;
      }
