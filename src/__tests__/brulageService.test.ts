...
describe("BrulageService", () => {
  ...
  describe("validateComposition", () => {
    it("should validate a correct composition", () => {
      ...
      const result = brulageService.validateComposition(
        composition,
        1,
        1,
        "aller"
    ...
    it("should reject composition with too many female players", () => {
      ...
      expect(result.errors.some((error) => error.type === "quota_female")).toBe(
        true
      );
    });
    ...
    it("should reject composition with too many foreign players", () => {
      ...
      const serviceWithExtraForeign = new BrulageService(
        mockBurnRecords,
        playersWithExtraForeign
      );
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.errors.some((error) => error.type === "quota_foreign")
      ).toBe(true);
    });
    ...
    it("should reject composition with incorrect ranking order", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.errors.some((error) => error.type === "ranking_order")
      ).toBe(true);
    });
    ...
    it("should reject burned player playing in higher team", () => {
      ...
      const result = serviceWithBurn.validateComposition(
        composition,
        2,
        2,
        "aller"
  ...
  describe("getAvailablePlayers", () => {
    it("should return all players when no restrictions", () => {
      ...
      expect(availablePlayers[0].points).toBeGreaterThanOrEqual(
        availablePlayers[1].points
      );
    });
    ...
    it("should exclude burned players", () => {
      ...
      const serviceWithBurn = new BrulageService([burnRecord], mockPlayers);
      const availablePlayers = serviceWithBurn.getAvailablePlayers(
        2,
        2,
        "aller"
      );
