...
# 2. Activer les règles de production (version améliorée)
echo "🔄 Activation des règles de production..."
cp firestore.prod.v2.rules firestore.rules
echo "✅ Règles de production activées (version améliorée)"
...
if [ $? -eq 0 ]; then
    ...
    if [ -f ".env.local" ]; then
        ...
        node test-rules-compatibility.js
    else
        echo "⚠️  Fichier .env.local non trouvé, tests de compatibilité ignorés"
        echo "💡 Créez un compte de test manuellement pour vérifier les permissions"
    fi
